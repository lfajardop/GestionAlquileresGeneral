(function(){
    ...
    function renderKpi(){
        ...
        $("#kpiUsuariosConContrato").text(usuarios.filter(x => Number(x.idContrato) > 0).length);
    }

    function contratosPorChofer(idChofer){
        return contratos.filter(x => Number(x.idChofer) === Number(idChofer));
    }

    function renderContratosSelect(idChofer, selectedContrato, target){
        const $target = $(target);
        clearSelect($target, "Sin contrato fijo");
        if(!idChofer){
            return;
        }
        contratosPorChofer(idChofer).forEach(item => {
            const estado = item.estado === "A" ? "Activo" : item.estado;
            $target.append(`<option value="${item.idContrato}">${item.numero} · ${item.placa} · ${estado}</option>`);
        });
        if(selectedContrato){
            $target.val(String(selectedContrato));
        }
    }
    ...
    function renderLista(){
        ...
        if(!usuarios.length){
            tbody.append('<tr><td colspan="9" class="fleet-empty">No hay usuarios mobile registrados todavía.</td></tr>');
            renderKpi();
            return;
        }
        ...
        usuarios.forEach(x => {
            tbody.append(`
                <tr>
                    ...
                    <td>${x.numeroContrato || "Sin contrato fijo"}</td>
                    <td>${x.placa || "-"}</td>
    ...
    function clearPanel(){
        ...
        $("#formCambiarContratoMobile")[0].reset();
    }
    ...
    function renderPanel(item){
        ...
        $("#detalleUsuarioContrato").text(item.numeroContrato || "Sin contrato fijo");
        $("#detalleUsuarioPlaca").text(item.placa || "-");
        ...
        renderContratosSelect(item.idChofer, item.idContrato, "#usuarioMobileContratoCambio");
    }
    ...
    function resetForm(){
        ...
        renderChoferes();
        renderContratosSelect(null, null, "#usuarioMobileContrato");
    }
    ...
    function fillForm(item){
        ...
        renderContratosSelect(item.idChofer, item.idContrato, "#usuarioMobileContrato");
        $("#usuarioMobileEstado").val(item.flgEstado || "A").prop("disabled", true);
    ...
    function payloadForm(){
        return {
            ...
            idContrato: $("#usuarioMobileContrato").val() ? Number($("#usuarioMobileContrato").val()) : null,
    ...
    function loadContratos(){
        return $.getJSON("/Flota/ContratosListar").done(r => {
            if(!r.success){
                notify(false, r.message || "No se pudieron cargar los contratos.");
                return;
            }
            contratos = r.data || [];
        });
    ...
    function reloadAll(){
        return $.when(loadCatalogos(), loadContratos(), loadUsuarios()).done(() => {
            if(modo === "crear"){
                resetForm();
            }
        });
    ...
    $("#usuarioMobileChofer").on("change", function(){
        renderContratosSelect($(this).val(), null, "#usuarioMobileContrato");
    });
    ...
    $("#formCambiarContratoMobile").on("submit", function(ev){
        ev.preventDefault();
        if(!usuarioActual){
            notify(false, "Selecciona un usuario mobile.");
            return;
        }
        postJson("/Flota/CambiarContratoUsuarioMobile", {
            idUsuarioMobile: usuarioActual.idUsuarioMobile,
            idContrato: $("#usuarioMobileContratoCambio").val() ? Number($("#usuarioMobileContratoCambio").val()) : null
        }).done(r => {
            notify(r.success, r.message);
            if(r.success){
                reloadAll();
            }
        }).fail(() => notify(false, "No se pudo cambiar el contrato fijo."));
    });

    $("#btnQuitarContratoUsuarioMobile").on("click", function(){
        if(!usuarioActual){
            notify(false, "Selecciona un usuario mobile.");
            return;
        }
        ...
        }).fail(() => notify(false, "No se pudo quitar el contrato fijo."));
    });
    ...
    $("#formUsuarioMobile").on("submit", function(ev){
        ...
        }).fail(() => notify(false, modo === "editar" ? "No se pudo actualizar el usuario mobile." : "No se pudo crear el usuario mobile."));
    });
