(function () {
    ...
    function setMotivoOptions(select, current) {
        ...
        const motivos = catalogos.motivosDia || [];
        if (!motivos.length) {
            $select.append($("<option>").val(current || "").text(current || "Sin catalogo"));
            return;
        }
    ...
    function renderReceiptDetail(d) {
        ...
        const dayRows = (d.dias || []).map(x => {
            const action = c.estadoCodigo !== "X"
                ? '<button class="btn btn-sm btn-outline-danger fa-remove-day" data-recibo="' + c.idReciboAlquiler + '" data-operacion="' + (x.idOperacionDia || 0) + '" data-fecha="' + String(x.fecha).substring(0, 10) + '">Quitar dia</button>'
                : '';
            return '<tr>' +
                '<td>' + date(x.fecha) + '</td>' +
                '<td>' + (x.origen === "M" ? "Manual" : "Operativo") + '</td>' +
            ...
            ? d.pagos.map(x => {
                const actionButtons = x.flgEstado !== "X"
                    ? '<div class="fa-row-actions">' +
                        '<button class="btn btn-sm btn-outline-primary fa-edit-payment" data-id="' + x.idPagoRecibo + '">Editar</button>' +
                        '<button class="btn btn-sm btn-outline-warning fa-validate-payment" data-id="' + x.idPagoRecibo + '">Validar</button>' +
                        '<button class="btn btn-sm btn-outline-danger fa-cancel-payment" data-id="' + x.idPagoRecibo + '">Anular</button>' +
                      '</div>'
    ...
    function selectedPayment(idPago) {
        return (detalleActual?.pagos || []).find(x => x.idPagoRecibo === Number(idPago));
    }
    ...
    function submitRemoveReceiptDay() {
        const payload = {
            idReciboAlquiler: Number($("#faRemoveDayReceiptId").val()),
            idOperacionDia: Number($("#faRemoveDayOperacionId").val()),
    ...
    $(document).on("click", ".fa-remove-day", function () {
        ...
        $("#faRemoveDayOperacionId").val($(this).data("operacion"));
    ...
    $(document).on("click", ".fa-edit-payment", function () {
        ...
        $("#faEditPaymentId").val(x.idPagoRecibo);
    ...
    $(document).on("click", ".fa-cancel-payment", function () {
        const x = selectedPayment($(this).data("id"));
        $("#faCancelPaymentId").val(x.idPagoRecibo);
    ...
    $(document).on("click", ".fa-validate-payment", function () {
        const x = selectedPayment($(this).data("id"));
        $("#faValidatePaymentId").val(x.idPagoRecibo);
    ...
    $("#faOpenCorrectDay").on("click", function () {
        ...
        if ($("#faCorrectDayCode").is("select")) {
            setMotivoOptions("#faCorrectDayCode", currentMotivo);
        } else {
            $("#faCorrectDayCode").val(currentMotivo);
        }
